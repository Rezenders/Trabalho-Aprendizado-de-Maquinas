...
def remove_html_tags(raw_html):
    clean_headers = re.compile('^[^<]+', flags=re.DOTALL)
    no_headers = re.sub(clean_headers, '', raw_html)

    cleanr = re.compile(r'<[^>]+>',flags=re.IGNORECASE)
    cleantext = re.sub(cleanr, '', no_headers)
    return cleantext
...
def remove_stop_words(clean_html):
    ...
    # Add words that are not usefull to the set
